...
# from PyQt5.QtCore.QValidator.QDoubleValidator import QDoubleValidator
...
class MainController(QMainWindow):
	...
	def __init__(self, folder):
		...
		# self.ui.chkBx_columnSec1.clicked.connect(lambda: self.call_3DColumn("gradient_bg"))
	...
	def get_columndata(self):
		...
		Returns: Selects column data from both Old & Integrated database

		"""
		loc = self.ui.combo_connLoc.currentText()
		columndata = get_columncombolist()
		old_columndata = get_oldcolumncombolist()
		combo_section = ''
		self.ui.combo_columnSec.addItems(columndata)
		combo_section = self.ui.combo_columnSec

		self.color_oldDatabase_section(old_columndata, columndata, combo_section)
	...
	# 	save_html(self.result, self.alist, self.column_data, fileName, report_summary, self.folder)
	...
	def disable_buttons(self):
		...
		self.ui.chkBx_columnSec1.setEnabled(False)
	...
	def enable_buttons(self):
		...
		self.ui.chkBx_columnSec1.setEnabled(True)
	...
	# 	self.column_data = self.fetchColumnPara()
	# 	column_column = CoverEndPlate(self.alist, self.resultObj, self.beam_data, self.folder)
	# 	status = self.resultObj['Bolt']['status']
	# 	if status is True:
	# 		if view != 'All':
	# 			if view == "Front":
	...
	def bolt_length_calculation(self, bolt_diameter):
		...
		column_data = self.fetchColumnPara()  # Fetches the column dimensions
		...
		length_required = self.bolt_head_thick_calculation(bolt_diameter) + self.nut_thick_calculation(bolt_diameter) + \
						  2.0 * float(flangeplatethickness) + float(column_T) + float(self.nut_thick_calculation(bolt_diameter)) + 10.0
...
def launch_column_coverplate_controller(osdagMainWindow, folder):
	...
	rawLogger = logging.getLogger("raw")
	rawLogger.setLevel(logging.INFO)
	# fh = logging.FileHandler("Connections\Moment\CCSpliceCoverPlate\CCSpliceCoverPlateBolted\coverplate.log", mode="w")
	file_handler = logging.FileHandler(os.path.join('Connections','Moment','CCSpliceCoverPlate','CCSpliceCoverPlateBolted', 'coverplate.log'), mode='w')
	formatter = logging.Formatter('''%(message)s''')
	file_handler.setFormatter(formatter)
	rawLogger.addHandler(file_handler)
	rawLogger.info(
		'''<link rel="stylesheet" type="text/css" href='''+ os.path.join('Connections','Moment','CCSpliceCoverPlate','CCSpliceCoverPlateBolted', 'log.css') +'''/>''')
	# ----------------------------------------------------------------------------
	module_setup()
	window = MainController(folder)
	osdagMainWindow.hide()
...
if __name__ == '__main__':
	...
	module_setup()
	# ----------------------------------------------------------------------------
	folder_path = "/home/reshma/Osdag_workspace/Coverplate"       #Todo
	if not os.path.exists(folder_path):
		os.mkdir(folder_path, 0o755)
	image_folder_path = os.path.join(folder_path, 'images_html')
	if not os.path.exists(image_folder_path):
	...
	window.show()
	sys.exit(app.exec_())
# launch_column_coverplate_controller()