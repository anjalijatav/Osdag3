    ...
    def member_capacity(self):
        ...
        if self.section.type == "Rolled":
            self.limitwidththkratio_flange = self.limiting_width_thk_ratio(column_f_t=self.section.flange_thickness,
                                                                           ...
                                                                           factored_axial_force=self.min_axial_load ,
        ...
        self.sum_IR = self.IR_axial +  self.IR_shear + self.IR_moment
        if  self.IR_axial < 0.3 and self.IR_moment < 0.5:
            self.min_axial_load = 0.3 * self.axial_capacity
            self.load_moment_min = 0.5 * self.section.moment_capacity
            logger.info("Axial load {} kN and Moment {} kN-m are less than minimun recommendation of IS 800:2007,"
                        " Cl.10.7.".format(round(self.min_axial_load/1000,2),round(self.load_moment_min/1000000,2) ))
            ...
            logger.info("Moment given is less than minimun recommendation of IS 800:2007,"
                        " Cl.10.7 {} kN-m" .format(round(self.load_moment_min/1000000,2 )))

        elif self.sum_IR <= 1.0 and self.IR_axial < 0.3:
            logger.info("Axial force given is less than minimun recommendation of IS 800:2007, Cl.10.7 {} kN.".format(round(self.min_axial_load/1000,2)))
